...
//                update the match in database
    ...
    @Override
    public List<List<String>> getPointTable(String tableURL) {
        List<List<String>> pointTable = new ArrayList<>();
        try {
            Document document = Jsoup.connect(tableURL).get();
            Elements table = document.select("div.point-table-grid");
            
            Element tableHeads = table.select("div").first();
            Elements divs = tableHeads.select("div>div");
            List<String> headers = new ArrayList<>();
            ...
            for(int i=1; i<table.size(); i++) {
            	List<String> points = new ArrayList<>();
            	if(table.get(i).hasAttr("class")) {
            		Elements tds = table.get(i).select("div>div");
		        	tds.forEach(div -> {
